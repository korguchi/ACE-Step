#!/usr/bin/env python3
"""Training script for ControlNet accompaniment generation."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train accompaniment generation model (ControlNet mode)"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    # Ensure ControlNet mode
    if config.accompaniment.mode != "controlnet":
        raise ValueError(
            f"This script is for ControlNet mode. "
            f"Config mode is: {config.accompaniment.mode}"
        )

    print("Training configuration:")
    print(f"  Mode: {config.accompaniment.mode}")
    print(f"  Output type: {config.accompaniment.output_type}")
    print(f"  Control encoder depth: {config.accompaniment.control_encoder_depth}")
    print(f"  Control encoder dim: {config.accompaniment.control_encoder_dim}")
    print(f"  ControlNet scale: {config.accompaniment.controlnet_scale}")
